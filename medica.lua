...
local item_patterns = {
  ["https?://([^/]*numerabilis%.u%-paris%.fr/medica/bibliotheque%-numerique/.*)$"] = "url",
  ["https?://([^/]*biusante%.parisdescartes%.fr/histmed/medica/.*)$"] = "url",
  ["https?://([^/]*numerabilis%.u%-paris%.fr/ressources/.+)$"] = "url",
}
...
allowed = function(url, parenturl)
  ...
  if string.match(url, "^https?://numerabilis%.u%-paris%.fr/iiif/") then
    return true
  end
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url)
    and status_code < 300
    and not string.match(url, "/ress?ources/")
    and not string.match(url, "/assets/")
    and not (
      string.match(url, "/iiif/")
      and not string.match(url, "%.json$")
    ) then
    html = read_file(file)
    if string.match(url, "https?://numerabilis%.u%-paris%.fr/iiif/.+/info%.json$") then
      local info = cjson.decode(html)
      local id = info["@id"]
      local format = "jpg"
      for _, profile_data in pairs(info["profile"]) do
        if type(profile_data) == "table"
          and type(profile_data["formats"]) == "table" then
          local found_jpg = false
          for _, profile_format in ipairs(profile_data["formats"]) do
            if profile_format == "jpg"
              or profile_format == "jpeg" then
              found_jpg = true
            end
          end
          if found_jpg then
            format = "jpg"
          elseif type(profile_data["formats"][1]) == "string" then
            format = profile_data["formats"][1]
          end
        end
      end
      check(id .. "/full/full/0/default." .. format)
      check(id .. "/full/full/0/default.tif")
      check(id .. "/full/full/0/default.jp2")
      for _, size_data in pairs(info["sizes"]) do
        local size_width = tonumber(size_data["width"])
        local size_height = tonumber(size_data["height"])
        check(id .. "/full/" .. size_width .. ",/0/default." .. format)
      end
      local image_width = tonumber(info["width"])
      local image_height = tonumber(info["height"])
      for _, tile_data in pairs(info["tiles"]) do
        local tile_width = tonumber(tile_data["width"])
        local tile_height = tonumber(tile_data["height"]) or tile_width
        for _, scale_factor in pairs(tile_data["scaleFactors"]) do
          local scale = tonumber(scale_factor)
          local region_width = tile_width * scale
          local region_height = tile_height * scale
          local y = 0
          while y < image_height do
            local current_region_height = math.min(region_height, image_height - y)
            local x = 0
            while x < image_width do
              local current_region_width = math.min(region_width, image_width - x)
              local size_width = math.ceil(current_region_width / scale)
              check(
                id .. "/" .. x .. "," .. y .. "," .. current_region_width
                .. "," .. current_region_height .. "/" .. size_width
                .. ",/0/default." .. format
              )
              x = x + region_width
            end
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["medica-br4qbu9n4qzn6od5"] = discovered_items,
    ["urls-x0vax238npzj81oj"] = discovered_outlinks
  }) do
